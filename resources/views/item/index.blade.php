                      ...
                      <tr>
                        ...
                        @if(auth()->user()->id==$post->user_id)
                          <td style="line-height: 3em;">                          
                            <a href="{{action('ItemController@edit',$post['id'])}}" class="btn btn-outline-primary d-block d-sm-inline mr-2" title='UPDATE'><i class="fa fa-edit"></i></button></a>
                            <form onsubmit="return confirm('Are you sure to delete?')" method="post" action="{{ action('ItemController@destroy', $post->id) }}" aria-label="{{__('Item deleted')}}" class="d-inline">
                              {{ csrf_field() }}
